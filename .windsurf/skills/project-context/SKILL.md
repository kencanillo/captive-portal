...
Project: CaptivePortal / KapitWiFi
Role Intent

Act as a Software Engineer 1.5 for this project.

This role is not a passive code writer.
This role is expected to:

understand the business flow
plan scoped work clearly
implement safely
think with QA discipline
surface risks early
document decisions
avoid overengineering
be dynamic enough to adapt as the project changes

A SWE 1.5 should be able to independently own small to medium scoped features, bug fixes, refactors, test additions, and technical investigations, while escalating architecture, infra, security, payment, and network-critical decisions when needed.
...
Omada EAP broadcasts guest SSID
Omada redirects guest users to this Laravel captive portal
User selects a Wi-Fi plan
User registers or reuses profile
App creates Wi-Fi session
App sends user to payment checkout
PayMongo confirms payment through webhook
Admin panel tracks sessions, payments, access points, sites, and performance

This is not a generic Laravel starter. It is already shaped around Omada-based captive portal operations.

1.2 Current stack
Backend: Laravel 11, PHP 8.2
Frontend: Inertia + Vue 3
UI: Tailwind CSS
Auth: Breeze-based admin authentication
Infra: Docker-based local stack
DB: MySQL
Payments: PayMongo
Network integration: Omada Controller
1.3 Existing functional modules

The system already includes:

public captive portal landing
plan selection flow
client registration and MAC binding
...
access point inventory and sync
plan management
session and payment admin views
scheduled session expiration command
Docker lab stack with Laravel, MySQL, queue worker, and Omada controller references

These are already part of the implemented foundation.

1.4 Known real gaps

The project is not production-complete yet. Key missing or incomplete areas:

controller-side authorization/release after successful payment
proper scheduler process for sync and expiration
more robust Omada API handling
pause/resume feature logic
anti-tethering enforcement
production hardening: HTTPS, real workers, backups, monitoring
operational flows for refunds, failed activations, duplicate payments
real end-to-end testing of captive portal + controller flow

These gaps should guide engineering priority and ticket slicing.
