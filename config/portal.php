<?php

return [
    'context_token_lifetime_minutes' => env('PORTAL_CONTEXT_TOKEN_LIFETIME_MINUTES', 10),
    'session_token_lifetime_minutes' => env('PORTAL_SESSION_TOKEN_LIFETIME_MINUTES', 120),
    'payment_token_lifetime_minutes' => env('PORTAL_PAYMENT_TOKEN_LIFETIME_MINUTES', 180),
    'allow_query_mac_fallback' => filter_var(env('PORTAL_ALLOW_QUERY_MAC_FALLBACK', false), FILTER_VALIDATE_BOOL),
    'bootstrap_timeout_seconds' => env('PORTAL_BOOTSTRAP_TIMEOUT_SECONDS', 8),
    'cache_store' => env('PORTAL_CACHE_STORE', env('CACHE_STORE', 'database')),
    'device_context_retry_after_ms' => env('PORTAL_DEVICE_CONTEXT_RETRY_AFTER_MS', 1500),
    'device_context_positive_cache_seconds' => env('PORTAL_DEVICE_CONTEXT_POSITIVE_CACHE_SECONDS', 300),
    'device_context_miss_cache_seconds' => env('PORTAL_DEVICE_CONTEXT_MISS_CACHE_SECONDS', 3),
    'device_transfer_cooldown_days' => env('PORTAL_DEVICE_TRANSFER_COOLDOWN_DAYS', 7),
    'single_active_device_per_client' => filter_var(env('PORTAL_SINGLE_ACTIVE_DEVICE_PER_CLIENT', false), FILTER_VALIDATE_BOOL),
    'bypass_payment' => filter_var(env('PORTAL_BYPASS_PAYMENT', false), FILTER_VALIDATE_BOOL),
    'ewallet_fee_rate' => env('PORTAL_EWALLET_FEE_RATE', 0.02),
    'omada_connect_timeout_seconds' => env('PORTAL_OMADA_CONNECT_TIMEOUT_SECONDS', 1),
    'omada_timeout_seconds' => env('PORTAL_OMADA_TIMEOUT_SECONDS', 4),
    'omada_mac_lookup_cache_seconds' => env('PORTAL_OMADA_MAC_LOOKUP_CACHE_SECONDS', 45),
    'omada_clients_snapshot_cache_seconds' => env('PORTAL_OMADA_CLIENTS_SNAPSHOT_CACHE_SECONDS', 5),
];
