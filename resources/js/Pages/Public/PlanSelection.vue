...
const showRegistrationForm = ref(!props.existingClient);
...
const payWithGCash = async (planId) => {
	...
	try {
		const payload = {
			...
			radio_id: props.portalContext?.radio_id || null,
	...
	<MainLayout title="KapitWiFi Portal">
		<section class="mx-auto max-w-2xl space-y-4">
			...
			<div v-else-if="showRegistrationForm" class="rounded-lg bg-white p-6 shadow">
				...
				<div class="mt-6 space-y-4">
					<div>
						...
						<input id="name" v-model="registrationForm.name" type="text"
					...
					<div>
						...
						<input id="phone_number" v-model="registrationForm.phone_number" type="tel"
							class="mt-1 w-full rounded-md border-slate-300 focus:border-slate-500 focus:ring-slate-500"
					...
					<div>
						...
						<input id="pin" v-model="registrationForm.pin" type="password"
							class="mt-1 w-full rounded-md border-slate-300 focus:border-slate-500 focus:ring-slate-500"
							placeholder="Create a 4-digit PIN" maxlength="10" required />
					...
					<div>
						...
						<input id="confirm_pin" v-model="registrationForm.confirm_pin" type="password"
							class="mt-1 w-full rounded-md border-slate-300 focus:border-slate-500 focus:ring-slate-500"
							placeholder="Confirm your PIN" maxlength="10" required />
					</div>
				</div>
			...
			<div v-else class="rounded-lg bg-white p-6 shadow">
				...
				<div v-if="props.portalContext?.site_name || props.portalContext?.ap_name || props.portalContext?.ssid_name"
				...
				<!-- MAC address section -->
				<div class="mt-4">
					<div v-if="props.portalContext?.mac_address"
					...
					<div v-else>
						<label class="block text-sm font-medium text-slate-700" for="manual_mac_address">MAC
							Address</label>
						<input id="manual_mac_address" v-model="manualMacAddress" type="text"
							class="mt-1 w-full rounded-md border-slate-300 focus:border-slate-500 focus:ring-slate-500"
							placeholder="AA:BB:CC:DD:EE:FF" pattern="^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})$" />
						<p class="mt-2 text-xs text-slate-500">Enter your device MAC address (format: AA:BB:CC:DD:EE:FF)
						</p>
						<p v-if="props.portalContext?.client_ip" class="mt-1 text-xs text-slate-500">Client IP: {{
							props.portalContext.client_ip }}</p>
					</div>
				</div>
			</div>
